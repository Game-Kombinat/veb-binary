...
namespace DataKombinat.Binary.Tags {
    ...
    public abstract class NbtTag : ICloneable {
        ...
        public abstract bool ReadTag(NbtBinaryReader readStream);

        public abstract void SkipTag(NbtBinaryReader readStream);

        public abstract void WriteTag(NbtBinaryWriter writeReader);

        // WriteData does not write the tag's ID byte or the name
        public abstract void WriteData(NbtBinaryWriter writeStream);
        ...
        public bool BoolValue {
            get {
                if (TagType == NbtTagType.Bool) {
                    return ((NbtBool)this).Value;
                } else {
                    throw new InvalidCastException("Cannot get BoolValue from " + GetCanonicalTagName(TagType));
                }
            }
        }
